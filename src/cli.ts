...
import { syncBookmarksGraphQL, syncGaps, syncBookmarkFolders, detectValidSessions } from './graphql-bookmarks.js';
import type { SyncProgress, GapFillProgress, FolderSyncProgress, BrowserSession } from './graphql-bookmarks.js';
...
export function engineOption(): Option {
  return new Option('--engine <name>', 'Override the LLM engine for this run (e.g. claude, codex, openai)');
}

export function modelOption(): Option {
  return new Option('--model <name>', 'Model name to use (for openai engine, defaults to gpt-4o)');
}

export function effortOption(): Option {
  return new Option('--effort <level>', 'Reasoning effort level (low, medium, high)');
}
...
export function buildCli() {
    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (useApi) {
          ...
          let { csrfToken, cookieHeader } = parseCookieOption(options.cookies);
          let selectedBrowser: string | undefined = options.browser ? String(options.browser) : undefined;

          if (!csrfToken && !selectedBrowser && !process.env.FT_BROWSER) {
            const sessions = await detectValidSessions();
            if (sessions.length > 1) {
              console.log('\n  Multiple browsers with active sessions found:');
              sessions.forEach((s, i) => {
                console.log(`    ${i + 1}. ${s.browser.displayName}`);
              });
              const answer = await promptText(`\n  Select a browser (1-${sessions.length}) or press enter for default: `, { output: process.stdout });
              if (answer.kind === 'interrupt') {
                throw new PromptCancelledError('Cancelled.', 130);
              }
              if (answer.kind === 'answer') {
                const idx = parseInt(answer.value, 10) - 1;
                if (idx >= 0 && idx < sessions.length) {
                  selectedBrowser = sessions[idx].browser.id;
                  csrfToken = sessions[idx].csrfToken;
                  cookieHeader = sessions[idx].cookieHeader;
                  console.log(`  Using ${sessions[idx].browser.displayName}...\n`);
                }
              }
            } else if (sessions.length === 1) {
              selectedBrowser = sessions[0].browser.id;
              csrfToken = sessions[0].csrfToken;
              cookieHeader = sessions[0].cookieHeader;
              console.log(`  Connected to session in ${sessions[0].browser.displayName}...\n`);
            }
          }
        ...
        const msg = (err as Error).message;
        const isCookieError = msg.includes('cookie') || msg.includes('Cookie') || msg.includes('Keychain') || msg.includes('Safe Storage') || msg.includes('secret-tool') || msg.includes('sqlite3') || msg.includes('profiles.ini');
        if (firstRun && isCookieError) {
          console.log(`
  ...
  Options:
    ...
    ft sync --browser chrome          Use Chrome
    ft sync --browser firefox         Use Firefox
    ft sync --cookies <ct0> <auth>    Pass cookies directly

  Troubleshooting:
    - Close your browser completely before syncing
    - Linux: Make sure you have \`sqlite3\` and \`libsecret-tools\` installed
    - Snap/Flatpak: If your browser is a Snap, ensure \`libsecret-tools\` is installed
`);
        } else {
          console.error(`\n  Error: ${msg}\n`);
        }
  ...
  program
    .command('classify')
    .description('Classify bookmarks by category and domain using LLM (requires claude/codex CLI or OpenAI-compatible API)')
  ...
  program
    ...
    .action(safe(async (engineArg?: string) => {
      ...
      if (available.length === 0) {
        ...
        console.log('  To use classification, you can:');
        ...
        console.log('    - Use an OpenAI-compatible API (LiteLLM, etc.) by setting OPENAI_API_KEY');
        return;
      }
