...
export async function detectValidSessions(): Promise<BrowserSession[]> {
  const installed = listInstalledBrowsers();
  const sessions: BrowserSession[] = [];

  for (const browser of installed) {
    try {
      if (browser.cookieBackend === 'firefox') {
        const profileDir = detectFirefoxProfileDir();
        const cookies = extractFirefoxXCookies(profileDir);
        sessions.push({ browser, csrfToken: cookies.csrfToken, cookieHeader: cookies.cookieHeader });
      } else {
        const dir = browserUserDataDir(browser);
        if (dir) {
          const cookies = extractChromeXCookies(dir, 'Default', browser);
          sessions.push({ browser, csrfToken: cookies.csrfToken, cookieHeader: cookies.cookieHeader });
        }
      }
    } catch {
      // Ignore browsers without active sessions
    }
  }
...
export async function syncBookmarksGraphQL(
  ...
  if (options.csrfToken) {
    ...
    // If no browser is specified, try all supported browsers until we find one with a session.
    if (!options.browser && !process.env.FT_BROWSER) {
      const sessions = await detectValidSessions();
      if (sessions.length > 0) {
        const session = sessions[0];
        csrfToken = session.csrfToken;
        cookieHeader = session.cookieHeader;
        console.log(`  \u2713 Connected to session in ${session.browser.displayName}`);
      } else {
        // Trigger a final attempt with the default browser to throw a helpful error
