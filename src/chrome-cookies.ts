...
import { existsSync, unlinkSync, copyFileSync, readFileSync, mkdtempSync, rmSync } from 'node:fs';
import { join, win32 as winPath, basename } from 'node:path';
...
function createCookieSnapshot(dbPath: string): { snapshotPath: string; cleanup: () => void } {
  const snapshotDir = mkdtempSync(join(tmpdir(), 'ft-chrome-cookies-'));
  const snapshotPath = join(snapshotDir, basename(dbPath));
  try {
    copyFileSync(dbPath, snapshotPath);
    // Best effort copy of WAL/SHM files
    try {
      if (existsSync(dbPath + '-wal')) copyFileSync(dbPath + '-wal', snapshotPath + '-wal');
      if (existsSync(dbPath + '-shm')) copyFileSync(dbPath + '-shm', snapshotPath + '-shm');
    } catch { /* skip if locked or missing */ }
    return {
      snapshotPath,
      cleanup: () => {
...
function queryDbVersion(dbPath: string): number {
  ...
  if (sqlite) {
    try {
      ...
      try {
        const row = db.prepare("SELECT value FROM meta WHERE key='version';").all()[0];
        return parseInt(String(row?.value ?? '0'), 10) || 0;
      } finally {
        db.close();
      }
    } catch { /* fall through to snapshot/binary */ }
  }
  ...
  try {
    ...
    const { snapshotPath, cleanup } = createCookieSnapshot(dbPath);
    try {
      return parseInt(tryQuery(snapshotPath), 10) || 0;
    } catch {
      return 0;
    } finally {
      cleanup();
...
function queryCookies(dbPath: string, domain: string, names: string[], browser: BrowserDef): { cookies: RawCookie[]; dbVersion: number } {
  ...
  const sql = `SELECT name, host_key, encrypted_value, value FROM cookies WHERE host_key LIKE '%${safeDomain}' AND name IN (${nameList});`;

  const doQuery = (p: string): RawCookie[] => {
    const sqlite = loadNodeSqlite();
    if (sqlite) {
      try {
        ...
        try {
          ...
          return rows.map((row: any) => ({
            ...
            encrypted_value_hex: row.encrypted_value instanceof Uint8Array 
              ? Buffer.from(row.encrypted_value).toString('hex') 
              : '',
        ...
        // If node:sqlite fails on the live file, we'll try again on the snapshot
        // if this was the first attempt. If it fails on the snapshot, we fall 
        // through to the sqlite3 binary.
      }
    }

    const binarySql = sql.replace('encrypted_value', 'hex(encrypted_value) as encrypted_value_hex');
    const out = execFileSync('sqlite3', ['-json', p, binarySql], {
      encoding: 'utf8',
      stdio: ['pipe', 'pipe', 'pipe'],
      timeout: 10000,
    }).trim();
  ...
  try {
    cookies = doQuery(dbPath);
  } catch (err: any) {
    const { snapshotPath, cleanup } = createCookieSnapshot(dbPath);
    try {
      cookies = doQuery(snapshotPath);
    } catch (e2: any) {
      const isMissingSqlite = err?.code === 'ENOENT' || e2?.code === 'ENOENT';
      const sqliteHint = isMissingSqlite && platform() === 'linux'
        ? '\nFix: Install sqlite3:  sudo apt-get install sqlite3\n'
        : '';
