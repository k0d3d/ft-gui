...
export async function classifyWithLlm(
  ...
  try {
    ...
    const batches: UnclassifiedBookmark[][] = [];
    ...
    const workers = Array(Math.min(concurrency, totalBatches)).fill(0).map(async () => {
      while (nextBatchIdx < totalBatches) {
        ...
        try {
          ...
          // Serialized database write
          await (dbWriteLock = dbWriteLock.then(() => {
            ...
            options.onBatch?.(processedBatches * BATCH_SIZE, totalUnclassified);

            // Save every 5 batches or on the last batch to balance safety and performance
          ...
          await (dbWriteLock = dbWriteLock.then(() => {
            failed += batch.length;
            processedBatches++;
            process.stderr.write(`  Batch ${batchIdx + 1} failed: ${(err as Error).message}\n`);
            options.onBatch?.(processedBatches * BATCH_SIZE, totalUnclassified);
          }));
        }
      }
    ...
    return { engine: engine.name, totalUnclassified, classified, failed, batches: totalBatches };
  } finally {
    db.close();
  }
}
...
export async function classifyDomainsWithLlm(
  ...
  try {
      ...
      while (nextBatchIdx < totalBatches) {
        ...
        try {
          ...
          await (dbWriteLock = dbWriteLock.then(() => {
            ...
            options.onBatch?.(processedBatches * BATCH_SIZE, total);
    ...
    return { engine: engine.name, totalUnclassified: total, classified, failed, batches: totalBatches };
  } finally {
    db.close();
  }
}
