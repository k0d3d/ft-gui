...
export type EngineConfig =
  ...
  | {
      type: 'api';
      invoke: (prompt: string, engine?: Pick<ResolvedEngine, 'model' | 'effort'>) => Promise<string>;
    };
...
const KNOWN_ENGINES: Record<string, EngineConfig> = {
  ...
  openai: {
    ...
    invoke: async (prompt, engine) => {
      ...
      if (!apiKey) {
        throw new Error(
          'OPENAI_API_KEY is not set. Classification with the openai engine requires an API key.\n' +
...
export async function resolveEngine(profile: EngineRunProfile = {}): Promise<ResolvedEngine> {
  ...
  if (requestedEngine) {
    ...
    const config = KNOWN_ENGINES[requestedEngine];
    if (config.type === 'cli' && !hasCommandOnPath(config.bin)) {
...
export function invokeEngine(engine: ResolvedEngine, prompt: string, opts: InvokeOptions = {}): string {
  if (engine.config.type === 'api') {
    throw new Error(`Engine "${engine.name}" is an API engine and must be called via invokeEngineAsync.`);
  }
...
export function invokeEngineAsync(engine: ResolvedEngine, prompt: string, opts: InvokeOptions = {}): Promise<string> {
  if (engine.config.type === 'api') {
    return engine.config.invoke(prompt, engine);
  }
